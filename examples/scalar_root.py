...
def test_output(etol=1e-8, ptol=1e-10):
    ...
    print('[Tangent Hyperbolas]', tangent_hyperbolas(f, df, d2f, -5, etol=etol, ptol=ptol))
...
def test_speed(etol=1e-8, ptol=1e-10, times=100):
    ...
    print(f'\n{"Bracketing":-^50}')
    timeit(bisect, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Bisect', number=times)
    timeit(regula_falsi, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Regula Falsi', number=times)
    timeit(illinois, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Illinois', number=times)
    timeit(pegasus, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Pegasus', number=times)
    timeit(anderson_bjorck, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Anderson-Bjork', number=times)
    timeit(dekker, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Dekker', number=times)
    timeit(brent, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Brent', number=times)
    timeit(chandrupatla, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Chandrupatla', number=times)
    timeit(ridders, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Ridders', number=times)
    timeit(toms748, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Toms748', number=times)
    timeit(wu, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='Wu', number=times)
    timeit(itp, args=(f, -10, 10), kwargs=dict(etol=etol, ptol=ptol),
           name='ITP', number=times)

    print(f'\n{"Quasi-Newton":-^50}')
    timeit(secant, args=(f, -10, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Secant', number=times)
    timeit(sidi, args=(f, [-10, -5, 0, 5, 10]), kwargs=dict(etol=etol, ptol=ptol),
           name='Sidi', number=times)
    timeit(steffensen, args=(f, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Steffensen', number=times)
    ...
    timeit(muller, args=(f, -10, -5, 0), kwargs=dict(etol=etol, ptol=ptol),
           name='Muller', number=times)

    print(f'\n{"Newton":-^50}')
    timeit(newton, args=(f, df, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Newton', number=times)
    timeit(halley, args=(f, df, d2f, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Halley', number=times)
    timeit(super_halley, args=(f, df, d2f, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Super-Halley', number=times)
    timeit(chebyshev, args=(f, df, d2f, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Chebyshev', number=times)
    timeit(tangent_hyperbolas, args=(f, df, d2f, -5), kwargs=dict(etol=etol, ptol=ptol),
           name='Tangent Hyperbolas', number=times)
